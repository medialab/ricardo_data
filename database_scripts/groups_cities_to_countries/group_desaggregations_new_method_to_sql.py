...
#KEY_TO_INSERT = ['source_label','original_reporting','currency','rate','year','partner','expimp','quality_tag','unit','reporting_continent','reporting_part_of_country','source','type','reporting_slug','reporting','original_partner','species_bullions','partner_type','partner_continent','partner_slug','spegen','reporting_type','notes','flow','partner_part_of_country','transport_type']
...
with open('group_desaggregations_new_method.json','r') as f:
    ...
    for desagg in group_desaggregations:
        ...
        for ratio in desagg['ratios']:
            ...
            insert_data.update(dict((k,v) for k,v in ratio.items() if 'partner' in k))

            insert_data['quality_tag']='group_desaggregation_%s_%s'%(desagg['method'],desagg['dyear'])
            # remove id
            del(insert_data['id'])
            if len(insert_data.keys()) != 28 :
    ...
    cursor.execute("""SELECT count(distinct partner) FROM flow_aggregated where partner_type = 'group'""")
    print("reducing %s partner groups "%cursor.fetchone())
    cursor.execute("""SELECT count(*) FROM flow_aggregated where partner_type = 'group'""")
    print( "reducing %s flows with partner groups "%cursor.fetchone())

    cursor.executemany("""INSERT INTO flow_aggregated ("%s") VALUES (%s)"""%('","'.join(sql_inserts_keys),','.join('?' for _ in range(28))),
        sql_inserts_values)
    ...
    # partners_to_complete_keys = ['type', 'continent', 'part_of_country']
