...
with open('group_desaggregations_new_method.json', 'r') as f:
    ...
    for desagg in group_desaggregations:
        ...
        for ratio in desagg['ratios']:
            ...
            insert_data.update(dict((k, v)
                                    for k, v in list(ratio.items()) if 'partner' in k))

            insert_data['quality_tag'] = 'group_desaggregation_%s_%s' % (
                desagg['method'], desagg['dyear'])
            # remove id
            del(insert_data['id'])
            if len(list(insert_data.keys())) != 28:
                print((len(list(insert_data.keys()))))
            else:
                sql_inserts_keys = list(insert_data.keys())  # always the same... this is bad
    ...
    cursor.executemany("""INSERT INTO flow_aggregated ("%s") VALUES (%s)""" % ('","'.join(sql_inserts_keys), ','.join('?' for _ in range(28))),
                       sql_inserts_values)
    print(("inserted %s new flows to disaggregated entities from groups" %
          cursor.rowcount))
    ...
    print(("Deleted %s flows which groups were desaggregated by ratio" %
          cursor.rowcount))
    ...
    cursor.execute(
        """SELECT count(*) FROM flow_aggregated where partner_type = 'group'""")
    print(("now flow_aggregated counts %s flows with partner groups " %
          cursor.fetchone()))
