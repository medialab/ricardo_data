...
def flow_in_pounds(flow): return flow['flow']*flow['unit']/flow['rate']
            ...
            try:
                ...
                if total and set(partners) == set(f[entity_key] for f in flows):
                    ...
                    ratios = [{'partner': f[entity_key],
                               ...
                               'partner_part_of_GPH_entity':f['%s_part_of_GPH_entity' % entity_key],
                               } for f in flows]
                    new_groups[y] = {
                        'ratios': ratios,
                        'ratio_flows': flows,
...
for (group, reporting), group_flows in itertools.groupby(cursor, lambda e: (e['partner'], e['reporting'])):
    ...
    direct_cases = {}
    subcursor.execute('SELECT * FROM flow_aggregated WHERE reporting = ? and partner IN ("' +
                      '","'.join(partners)+'") ORDER BY year', [reporting])
    ...
    by_direct_method = process_nearest_ratio(
        group_flows, direct_flows_complete)
    ...
    if len(by_direct_method['remaining_group_flows']) > 0:
        # looking for mirror flows
        subcursor.execute('SELECT * FROM flow_aggregated WHERE reporting IN ("' +
                          '","'.join(partners)+'") AND partner = ? ORDER BY year', [reporting])
        # group flows by year and expimp
...
with open('group_desaggregations_new_method.json', 'w', encoding='utf8') as f:
