...
with open("group_desaggregations.json", "r") as f:
    ...
    for desagg in group_desaggregations:
        ...
        for new_flow in (
            d for d in desagg["new_flows"] if "negative" not in d["quality_tag"]
        ):
        ...
        if desagg["delete_group_flow"]:
            ...
            desagg["update"] != {} and "negative" not in desagg["update"]["quality_tag"]
        ):
            if desagg["update"]["quality_tag"] == "group_desaggregation_total":
                partners_to_complete.append(desagg["update"]["partner"])
            sql_updates_keys = list(desagg["update"].keys())
            sql_updates_values.append(list(desagg["update"].values()) + [id_flow])
    ...
    print(
        "inserted %s new flows to disaggregated entities from groups" % cursor.rowcount
    )

    cursor.executemany(
        """UPDATE flow_aggregated SET %s WHERE id=?"""
        % ",".join("%s=?" % k for k in sql_updates_keys),
        sql_updates_values,
    )
    ...
    for partner in cursor:
        partners_to_complete_values.append(
            [v for k, v in partner.items() if k in partners_to_complete_keys]
            + [partner["RICname"]]
        )

    cursor.executemany(
        """UPDATE flow_aggregated SET %s WHERE partner=?"""
        % ",".join('"partner_%s"=?' % k for k in partners_to_complete_keys),
        partners_to_complete_values,
    )
    print(
        "Added metadata to %s  partner desaggregated from group"
        % len(partners_to_complete)
    )
