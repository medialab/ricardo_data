...
for (group, reporting), flows in itertools.groupby(group_flows, lambda e: (e['partner'], e['reporting'])):
    ...
    year_expimp = dict((year, dict((f['expimp'], f) for f in yf))
    ...
    for mirror in mirror_flow:
        ...
        if year in year_expimp and impexp in year_expimp[year]:
            key = "%s,%s" % (year, impexp)
            mirror_cases[key] = mirror_cases[key] + \
                [mirror] if key in mirror_cases else [mirror]
    ...
    for k, mirrors in mirror_cases.iteritems():
        ...
        if len(coverage) == len(partners):
            ...
            new_flow = year_expimp[year][expimp]['flow'] - sum(flow_in_pounds(
                m) for m in mirrors)*year_expimp[year][expimp]['rate']/year_expimp[year][expimp]['unit']
