...
for (group, reporting), flows in itertools.groupby(
    group_flows, lambda e: (e["partner"], e["reporting"])
):
    flows = list(flows)

    nb_flows_with_groups += len(flows)
    year_expimp = dict(
        (year, dict((f["expimp"], f) for f in yf))
        for year, yf in itertools.groupby(flows, lambda e: e["year"])
    )
    ...
    mirror_flow.execute(
        ...
        + '","'.join(partners)
        + '") AND partner = ? AND year >= ? AND year<= ? ORDER BY year',
    ...
    for mirror in mirror_flow:
        ...
        if year in year_expimp and impexp in year_expimp[year]:
            key = "%s,%s" % (year, impexp)
            mirror_cases[key] = (
                mirror_cases[key] + [mirror] if key in mirror_cases else [mirror]
            )

    complete_desagg_ratio = 0
    complete_desagg = 0
    group_modif = 0
    for k, mirrors in mirror_cases.items():
        (year, expimp) = k.split(",")
        year = int(year)
        ...
        if len(coverage) == len(partners):
            ...
            if mirror_total == 0:
                ...
                for mirror in mirrors:
                    ...
                    group_desaggregation["new_flows"].append(
                        year_expimp[year][expimp].copy()
                    )
                    # the partner is not the group but a precise partner
                    group_desaggregation["new_flows"][-1]["partner"] = mirror[
                        "reporting"
                    ]
                    group_desaggregation["new_flows"][-1]["partner_type"] = mirror[
                        "reporting_type"
                    ]
            ...
            new_flow = (
                year_expimp[year][expimp]["flow"]
                - sum(flow_in_pounds(m) for m in mirrors)
                * year_expimp[year][expimp]["rate"]
                / year_expimp[year][expimp]["unit"]
            )
...
print(
    "%s flows desaggregated on %s %.2f"
    % (
        nb_flows_with_groups_desagg,
        nb_flows_with_groups,
        nb_flows_with_groups_desagg / nb_flows_with_groups,
    )
)
print(
    "with ratio:%s, desagg:%s, modified:%s"
    % (complete_desagg_ratio_tot, complete_desagg_tot, group_modif_tot)
)


with open("group_desaggregations.json", "w") as f:
    json.dump(group_desaggregations, f, indent=2)
