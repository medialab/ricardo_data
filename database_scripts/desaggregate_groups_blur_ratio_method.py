...
def flow_in_pounds(flow):
    return flow["flow"] * flow["unit"] / flow["rate"]


def group_flows_by_yearexpimp(flows, method):
    grouped_flows = {"Exp": defaultdict(list), "Imp": defaultdict(list)}
    for flow in flows:
        year = flow["year"]
        if method == "direct":
            impexp = flow["expimp"]
...
def filter_partial_partners_grouped_flows(grouped_flows, partners, method):
    def filter_groups(groups):
        new_groups = {}
        for y, flows in list(groups.items()):
...
def process_nearest_ratio(group_flows, ratio_flows):
    ...
    for flow in group_flows:
        ...
        ratio = None
        for ratio_year, ratio_group in list(ratio_flows[expimp].items()):
...
def flow_disaggregate_groups():
    ...
    for (group, reporting), group_flows in itertools.groupby(
        cursor, lambda e: (e["partner"], e["reporting"])
    ):
        subcursor.execute(
            "SELECT RICname_part FROM RICentities_groups WHERE RICname_group = ?",
            [group],
        )
        partners = set((row["RICname_part"] for row in subcursor))

        # looking for mirror flows
        direct_cases = {}
        subcursor.execute(
            'SELECT * FROM flow_aggregated WHERE reporting = ? and partner IN ("'
            + '","'.join(partners)
            + '") ORDER BY year',
            [reporting],
        )
        ...
        direct_flows_complete = filter_partial_partners_grouped_flows(
            direct_flows_groups, partners, "direct"
        )
        ...
        if len(by_direct_method["remaining_group_flows"]) > 0:
            # looking for mirror flows
            subcursor.execute(
                'SELECT * FROM flow_aggregated WHERE reporting IN ("'
                + '","'.join(partners)
                + '") AND partner = ? ORDER BY year',
                [reporting],
            )
            # group flows by year and expimp
            mirror_flows_groups = group_flows_by_yearexpimp(subcursor, "mirror")
            # remove expimp-years for which we don't have all partners
            mirror_flows_complete = filter_partial_partners_grouped_flows(
                mirror_flows_groups, partners, "mirror"
            )
            # add direct ratios to group flows
            by_mirror_method = process_nearest_ratio(
                by_direct_method["remaining_group_flows"], mirror_flows_complete
            )
    ...
    print(
        (
            "%s flows with group %s %.2f pc desaggregated by direct method, %s %.2f pc desaggregated by mirror method"
            % (
                total_flows,
                total_direct_methods,
                100 * float(total_direct_methods) / total_flows,
                total_mirror_methods,
                100 * float(total_mirror_methods) / total_flows,
            )
        )
    )
    ...
    # write json
    print(
        "Write a group disaggregations report in ./out_data/group_desaggregations_new_method.json"
    )
    with open(
        "./out_data/group_desaggregations_new_method.json", "w", encoding="utf8"
    ) as f:
        json.dump(desaggregated_group_flows, f, indent=2)
    ...
    for desagg in desaggregated_group_flows:
